...
graph TB
    subgraph MAIN["🎯 TASK MANAGEMENT APPLICATION"]
        subgraph BACKEND["🔧 BACKEND (FastAPI + Python)"]
            ...
            BE_TASKS["📄 backend/tasks.py (70 lines)<br/>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━<br/>📋 Task class (L5-20):<br/>    ├── __init__(id, description) (L8-12)<br/>    ├── mark_completed() → void (L14-15)<br/>    └── __repr__() → str with ✓/○ (L17-19)<br/>    📊 ATTRIBUTES:<br/>    ├── id: int<br/>    ├── description: str<br/>    ├── completed: bool = False<br/>    └── created_at: datetime.now()<br/><br/>🎯 TaskManager class (L23-70):<br/>    ├── __init__() → empty list + _next_id=1 (L26-28)<br/>    ├── add_task(desc) → Task (L30-34)<br/>    ├── list_tasks() → List[Task] (L36-38)<br/>    ├── get_task(id) → Optional[Task] (L40-44)<br/>    ├── complete_task(id) → bool (L46-51)<br/>    ├── remove_task(id) → bool (L52-57)<br/>    ├── get_pending_tasks() → List[Task] (L59-60)<br/>    └── get_completed_tasks() → List[Task] (L62-63)"]
        end
